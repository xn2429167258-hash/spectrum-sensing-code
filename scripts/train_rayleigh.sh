...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: bash scripts/train_rayleigh.sh [--date-tag YYYYMMDD] [--seed N] [--run-suffix NAME] [--batch-size N]"
            exit 0 ;;
...
SAVE_ROOT="runs/rayleigh_${DATE_TAG}_${RUN_SUFFIX}"
SRC="data/source_gaussian_qpsk_rxshift_100mhz.pkl"
TGT="data/target_rayleigh_qpsk_rxshift_100mhz.pkl"

[[ -f "$SRC" ]] || { echo "Source data file not found: $SRC" >&2; exit 1; }
[[ -f "$TGT" ]] || { echo "Rayleigh target data file not found: $TGT" >&2; exit 1; }
mkdir -p "$SAVE_ROOT"
...
COMMON_DA_ARGS=(
    ...
    --target-family rayleigh --target-domain hard --target-path "$TGT"
    --save-root "$SAVE_ROOT" --seed "$SEED" --batch-size "$BATCH_SIZE"
    --fft-norm-mode log_power --encoder-norm bn --encoder-dilations 1,1,1 --encoder-use-se
...
run_cmd "05 print main evaluation" python evaluate_main.py --eval-domains rayleigh --source-checkpoint "$SOURCE_CKPT" --osada-checkpoint "$FULL_CKPT" --rayleigh-path "$TGT" --batch-size "$BATCH_SIZE" --no-print-per-snr
run_cmd "06 print ablation evaluation" python evaluate_ablation.py --target-path "$TGT" --domain-name Rayleigh --da-occ-checkpoint "$OCC_CKPT" --occ-pseudo-checkpoint "$OCC_PSEUDO_CKPT" --full-osada-checkpoint "$FULL_CKPT" --batch-size "$BATCH_SIZE" --no-print-per-snr
