param(
    ...
    [int]$Seed = 42,
    [string]$RunSuffix = "pc_rayleigh_seed42",
    [int]$BatchSize = 32,
    [string]$SourcePath = "data/source_gaussian_qpsk_rxshift_100mhz.pkl",
    [string]$TargetPath = "data/target_rayleigh_qpsk_rxshift_100mhz.pkl",
    [string]$SaveRoot = "",
...
if ([string]::IsNullOrWhiteSpace($SaveRoot)) {
    $SaveRoot = Join-Path $ProjectRoot ("runs\\rayleigh_" + $DateTag + "_" + $RunSuffix)
}
New-Item -ItemType Directory -Force -Path $SaveRoot | Out-Null

if (-not (Test-Path $SourcePath)) { throw "Source data file not found: $SourcePath" }
if (-not (Test-Path $TargetPath)) { throw "Rayleigh target data file not found: $TargetPath" }
...
$CommonDaArgs = @(
    ...
    "--source-family", "gaussian", "--source-path", $SourcePath,
    "--target-family", "rayleigh", "--target-domain", "hard", "--target-path", $TargetPath,
    "--save-root", $SaveRoot, "--seed", [string]$Seed, "--batch-size", [string]$BatchSize,
    "--fft-norm-mode", "log_power", "--encoder-norm", "bn", "--encoder-dilations", "1,1,1", "--encoder-use-se",
...
if (-not $SkipEvaluation) {
    Invoke-Step "05 print main evaluation" @("evaluate_main.py", "--eval-domains", "rayleigh", "--source-checkpoint", $SourceCkpt, "--osada-checkpoint", $FullCkpt, "--rayleigh-path", $TargetPath, "--batch-size", [string]$BatchSize, "--no-print-per-snr")
    Invoke-Step "06 print ablation evaluation" @("evaluate_ablation.py", "--target-path", $TargetPath, "--domain-name", "Rayleigh", "--da-occ-checkpoint", $OccCkpt, "--occ-pseudo-checkpoint", $OccPseudoCkpt, "--full-osada-checkpoint", $FullCkpt, "--batch-size", [string]$BatchSize, "--no-print-per-snr")
}
